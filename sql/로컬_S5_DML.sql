...
;
-- '회원/정보관리''주문/결제', '배송', '반품/환불/교환/AS', '영수증/증빙서류', '상품/이벤트', '기타'

--DESC BOARD;
--INSERT INTO BOARD(BOARD_NO,BOARD_TITLE,BOARD_CONTENT,BOARD_WRITER,BOARD_DATE,BOARD_CATEGORY_CODE)
--VALUES ((SELECT NVL(MAX(BOARD_NO),0)+1 FROM BOARD),'제목','내용','박민지',1,(SELECT NVL(MAX(BOARD_CATEGORY_CODE),0)+1 FROM BOARD_CATEGORY))
--;
